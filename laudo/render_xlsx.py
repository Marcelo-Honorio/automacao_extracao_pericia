...
TEMPLATE = "laudo\\templates\\template.xlsx"
SHEET = "ANEXO 2"
...
def preencher_tabela(ws, df):
    ...
    for i, row in df.iterrows():
        ...
        ws.cell(r, 6, row["dias"])
        ws.cell(r, 7, row["dias_acum"])
        ws.cell(r, 8, row["snd"])
        ws.cell(r, 9, row["sna"])
        ws.cell(r, 10, row["snm"])
        ws.cell(r, 11, row["juros"])
        ws.cell(r, 12, row["tx_mensal"])
        ws.cell(r, 13, row["tx_anual"]) ### corrigir p/ incluir tx de mercado
        ws.cell(r, 14, row["Historico"])
        ws.cell(r, 15, row["debito_recal"])
        ws.cell(r, 16, row["estorno_credito"])
        ws.cell(r, 17, row["saldo_recal"])
        ws.cell(r, 18, row["snd"])
        ws.cell(r, 19, row["sna"])
        ws.cell(r, 20, row["snm"])
        ws.cell(r, 21, row["juros_recal"])
...
def gerar_relatorio(df, dados_cabecalho, resumo):
